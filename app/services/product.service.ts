import type { ProductDetailResponse, ProductListItem, ProductListParams, ProductListResponse } from "../models/product.model";

const getApiBaseUrl = () => (process.env.API_URL ?? process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:8000/api").replace(/\/$/, "");
...
export async function getProducts(params: ProductListParams = {}): Promise<ProductListResponse> {
  ...
  const response = await fetch(`${getApiBaseUrl()}/products?${query}`, { headers: { Accept: "application/json", lang: "vi" }, cache: "no-store" });
  ...
  if (!Array.isArray(payload.data) || !payload.meta) throw new Error("Dữ liệu danh sách sản phẩm không đúng định dạng");
  return payload as ProductListResponse;
}

export async function getProduct(id: number): Promise<ProductListItem> {
  const response = await fetch(`${getApiBaseUrl()}/products/${id}`, { headers: { Accept: "application/json", lang: "vi" }, cache: "no-store" });
  ...
  if (!payload.data || typeof payload.data.id !== "number") throw new Error("Dữ liệu chi tiết sản phẩm không đúng định dạng");
  return payload.data;
}
